...
SDirectionalLightComponent::SDirectionalLightComponent(std::string sComponentName,
	DirectX::BoundingSphere* pLevelBounds, UINT iShadowMapOneDimensionSize) : SLightComponent(sComponentName)
{
	lightType = SLightComponentType::SLCT_DIRECTIONAL;
	this->pLevelBounds = pLevelBounds;
...
void SDirectionalLightComponent::deallocateShadowMaps(std::vector<std::unique_ptr<SFrameResource>>* vFrameResources)
{
	for (size_t i = 0; i < vFrameResources->size(); i++)
	{
		bool bExpanded; // don't care, we anyway update CB on every frame
...
void SDirectionalLightComponent::updateCBData(SFrameResource* pCurrentFrameResource)
{
	using namespace DirectX;

	XMVECTOR lightDir = DirectX::XMLoadFloat3(&lightProps.vDirection);
	XMVECTOR lightPos = -2.0f * pLevelBounds->Radius * lightDir;
	XMVECTOR targetPos = XMLoadFloat3(&pLevelBounds->Center);
	...
	// Ortho frustum in light space encloses level.
	float l = sphereCenterLS.x - pLevelBounds->Radius;
	float b = sphereCenterLS.y - pLevelBounds->Radius;
	float n = sphereCenterLS.z - pLevelBounds->Radius;
	float r = sphereCenterLS.x + pLevelBounds->Radius;
	float t = sphereCenterLS.y + pLevelBounds->Radius;
	float f = sphereCenterLS.z + pLevelBounds->Radius;
...
void SDirectionalLightComponent::renderToShadowMaps(ID3D12GraphicsCommandList* pCommandList, SFrameResource* pCurrentFrameResource, SRenderPassConstants* pRenderPassCB)
{
...
void SDirectionalLightComponent::finishRenderToShadowMaps(ID3D12GraphicsCommandList* pCommandList)
{
